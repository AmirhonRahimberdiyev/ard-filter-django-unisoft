...
class DjangoTelegramBot:
    transfer_steps = ("from_card", "to_card", "amount", "comment")
    ...
    def _finish_share_money(self, chat_id: int, session: ChatSession) -> None:
        source_card = session.transfer_payload["from_card"]
        destination_card = session.transfer_payload["to_card"]
        ...
        try:
            with transaction.atomic():
                source_card = self.card_model.objects.select_for_update().get(pk=source_card.pk)
                destination_card = self.card_model.objects.select_for_update().get(pk=destination_card.pk)

                source_balance = Decimal(str(getattr(source_card, balance_field.name)))
                destination_balance = Decimal(str(getattr(destination_card, balance_field.name)))
                if amount > source_balance:
                    raise ValidationProblem(
                        self._text(
                            chat_id,
                            "Во время перевода баланс изменился. Попробуйте снова.",
                            "Balance changed during the transfer. Please try again.",
                        )
                    )
                setattr(source_card, balance_field.name, source_balance - amount)
                setattr(destination_card, balance_field.name, destination_balance + amount)

                source_card.full_clean()
                destination_card.full_clean()
                source_card.save(update_fields=[balance_field.name])
                destination_card.save(update_fields=[balance_field.name])
        except (self.card_model.DoesNotExist, ValidationProblem, ValidationError) as exc:
            self._reset_flow_state(session)
            if isinstance(exc, self.card_model.DoesNotExist):
                details = self._text(
                    chat_id,
                    "одна из карт больше не существует.",
                    "one of the cards no longer exists.",
                )
            ...
            self.client.send_message(
                ...
                self._text(
                    chat_id,
                    f"Не удалось выполнить перевод: {details}\nИспользуйте /sharemoney, чтобы попробовать снова.",
                    f"Could not complete the transfer: {details}\nUse /sharemoney to try again.",
                ),
            )
            return
        ...
        self.client.send_message(
            ...
            self._text(
                ...
                f"Сумма: {amount}\n"
                f"Новый баланс отправителя: {getattr(source_card, balance_field.name)}",
                "Transfer completed successfully.\n"
                f"From: {self._card_label(source_card)}\n"
                f"To: {self._card_label(destination_card)}\n"
                f"Amount: {amount}\n"
                f"New source balance: {getattr(source_card, balance_field.name)}",
            ),
        )
    ...
    def _format_card_details(self, card: Model) -> str:
        ...
        if number_field is not None:
            lines.append(f"{number_field.verbose_name}: {self._mask_card_number(getattr(card, number_field.name, ''))}")
    ...
    def _card_label(self, card: Model) -> str:
        ...
        if number_field is not None:
            ...
            if value:
                return f"#{card.pk} {self._mask_card_number(str(value))}"
        return f"#{card.pk}"

    def _mask_card_number(self, value: str) -> str:
    ...
    def _allowed_card_prefixes(self) -> tuple[str, ...]:
        raw_prefixes = str(
            _get_setting("TELEGRAM_CARD_ALLOWED_PREFIXES", default="8600,9860")
        )
        prefixes = tuple(
    ...
    def _card_luhn_required(self) -> bool:
        raw_value = str(_get_setting("TELEGRAM_CARD_REQUIRE_LUHN", default="0")).lower()
    ...
    def _normalize_card_number(self, chat_id: int, label: str, raw_value: str) -> str:
        ...
        if len(digits) != 16:
            raise ValidationProblem(
                self._validation_message(
                    ...
                    "karta raqami aynan 16 ta raqamdan iborat bo'lishi kerak.",
                )
            )

        if not CARD_NUMBER_RE.match(digits):
            raise ValidationProblem(
                self._validation_message(
                    chat_id,
                    label,
        ...
        if self._card_luhn_required() and not self._passes_luhn(digits):
            raise ValidationProblem(
                self._validation_message(
                    chat_id,
                    label,
                    "номер карты не прошел контрольную проверку.",
                    "card number did not pass the checksum validation.",
                    "karta raqami nazorat tekshiruvidan o'tmadi.",
                )
            )
        ...
        if digits.startswith("998") and len(digits) == 12:
            ...
            raise ValidationProblem(
                self._validation_message(
                    chat_id,
                    label,
                    "телефон должен быть в формате +998901234567 или 901234567.",
    ...
    def _get_user_cards(self, chat_id: int):
        owner_field = self.card_field_map.get("owner")
    ...
    def _build_card_field_map(self) -> dict[str, Any]:
        ...
        return {
            "owner": self._find_field(fields, ("telegram_chat_id", "chat_id", "telegram")),
